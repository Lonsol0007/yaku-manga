package yaku.ui.reader.viewer.webtoon
...
import yaku.databinding.ReaderErrorBinding
import yaku.i18n.MR
...
class WebtoonPageHolder(
    ...
    private suspend fun process(imageSource: BufferedSource): BufferedSource {
        ...
        val translated = if (page?.translationRequested?.value == true) {
        ...
        if (viewer.config.dualPageSplit) {
            val isDoublePage = ImageUtil.isWideImage(translated)
            if (isDoublePage) {
                val upperSide = if (viewer.config.dualPageInvert) ImageUtil.Side.LEFT else ImageUtil.Side.RIGHT
                return ImageUtil.splitAndMerge(translated, upperSide)
            }
        }
