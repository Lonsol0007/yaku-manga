...
object TelemetryConfig {
    ...
    fun init(context: Context) {
        ...
        if (!context.isProductionApp()) return
    ...
    private fun Context.isProductionApp(): Boolean {
        if (packageName !in PRODUCTION_PACKAGES) return false

        return packageManager.getPackageInfo(packageName, SignatureFlags)
            .getCertificateFingerprints()
            .any { it == PRODUCTION_CERTIFICATE_FINGERPRINT }
    }
}

private val PRODUCTION_PACKAGES = hashSetOf("app.yaku", "app.yaku.debug")
private const val PRODUCTION_CERTIFICATE_FINGERPRINT =
